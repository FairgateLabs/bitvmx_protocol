...
class ExecutionChallengeScriptListGeneratorService:
    ...
    def __call__(
        ...
        current_hardcoded_instruction_without_input = current_hardcoded_instruction[
            (9 * 8 + 2) * 2 :
        ]
        # current_hardcoded_values = current_hardcoded_instruction[:(9*8+2)*2]

        execution_script = BitcoinScript.from_raw(
            scriptrawhex=current_hardcoded_instruction_without_input, has_segwit=True
        )
        ...
        script.append(1)
        return BitcoinScriptList([script] * 763)
